# -*- coding: utf-8 -*-
"""
Created on Sun Sep  4 10:19:04 2022
...
def RSIScanner():
    ...
    try:
        ...
        period, long_entry, short_entry = params.GetSignalPara_RSI('RSISignalParams.csv')
        
        # get the trade parameters
        capital, max_capital_deploy, buy_margin, sell_margin, pnl_target, pnl_stoploss, trading_cost = params.GetTradePara('RSITradeParams.csv')
        
        # get the scan list
        # API dependent
        tradescanlist = myutils.read_dataframe('RSIScanList.csv')
        
        # get the strategy state
        strategy_state = myutils.read_dataframe('RSIStrategyState.csv')
        
        current_time = dt.datetime.now().time()
        print(current_time,stoptime)
        ...
        while (current_time < stoptime):
            ...
            for stocknum in range(len(tradescanlist)):
                stockname = tradescanlist.iloc[stocknum+4, sclcol_symbolname]
                
                print('\nChecking : ' + stockname)
                
                is_order_executing = strategy_state.iloc[stocknum+4, sscol_entry_executing] + strategy_state.iloc[stocknum, sscol_exit_executing]
                print(strategy_state.iloc[stocknum, sscol_exit_executing])
                print(strategy_state.iloc[stocknum,sscol_entry_executing])
                if (is_order_executing == 0):
                    # no trades are currentlypending execution on this stock
                    
                    spotsymbol = str(tradescanlist.iloc[stocknum+4, sclcol_spotsymbol])
                    
                    # make API call to get live spot LTP
                    # API dependent
                    spotltp = jp.GetQuote(spot_exchange, spotsymbol)
                    print('Quote Price', spotltp['LTP'])
    ...
    #                    continue
                    ...
                    current_signal = sg.generate_signal_RSI(stockname, spotltp, period, long_entry, short_entry, '')
                    ...
                    lotsize = tradescanlist.iloc[stocknum+4, sclcol_lotsize]
                    
                    # API dependent
                    futsymbol = str(tradescanlist.iloc[stocknum+4, sclcol_symbol])
                    
                    # make API call to get live Futures LTP
                    # API dependent
                    futltp = jp.GetQuote(fo_exchange, futsymbol)
                    print('Get quote ', futltp)
    ...
    #                    continue
                    ...
                    if ((current_signal != 0)  ):
                        ...
                        prevexitreason  = ''
                        print(futltp, current_signal, current_pos, prev_signal, capital, max_capital_deploy, buy_margin, sell_margin,pnl_target, pnl_stoploss, mtm_pl, lotsize, prevexitreason)
    ...
    #                    
            ...
            myutils.write_dataframe('RSIStrategyState.csv', strategy_state)
